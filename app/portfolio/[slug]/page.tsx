...
import { PORTFOLIOS } from "../components/PortfolioTabs/components/constants";
import { useParams, useRouter } from "next/navigation";

const page: React.FC = () => {
  const { slug } = useParams<{ slug: string }>();
  const navigate = useRouter();
  const portfolio = PORTFOLIOS.find((p) => p.link === slug);
            ...
            <div className="md:w-[88%] w-[92%]">
              <div className="flex justify-center flex-col w-full h-full items-center gap-3">
                <p className="flex items-center gap-3 font-light">
                  ...
                  <Link href="/portfolio">
                    <span className="text-[#ef41de]">Portfolio </span>
                  </Link>{" "}
                  <ArrowRightIcon className="w-3 h-3 text-white" />{" "}
